...
# ---- Total demo time ----
TOTAL_TEST_MINUTES = 60
# baseline gets first half, scheduled gets second half
...
AVG_JOBS_PER_MINUTE = 3.0
TRAINING_RATIO = 0.75
INFERENCE_RATIO = 0.25

TRAINING_DURATION_RANGE_S = (120, 300)
TRAINING_FLEXIBLE_PROB = 0.95
TRAINING_DEFER_SLACK_RANGE_S = (180, 600)

INFERENCE_DURATION_RANGE_S = (20, 60)
INFERENCE_DUTY_CYCLE_CHOICES = [0.20, 0.30, 0.40]

# ---- Thermal policy ----
GPU_TEMP_BLOCK_C = 68.0
GPU_TEMP_ALERT_C = 66.0

# ---- Mini compressed peak window inside each phase ----
# fractions of EACH PHASE, not of whole test
PEAK_START_FRAC = 0.15
PEAK_END_FRAC = 0.85

CHEAP_RATE_PER_KWH = 0.15
PEAK_RATE_PER_KWH = 0.50
...
# ---- Scaling assumptions for “full-size DC” estimate ----
SCALING_NODE_COUNT = 1000
ANNUALIZED_HOURS = 24 * 365
POWER_USAGE_EFFECTIVE_HOURS_PER_DAY = 24

# ---- Output ----
OUTPUT_ROOT = Path("/home/paul/gpu-burn/demo_compare_outputs")
RUN_NAME = f"demo_compare_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
...
def get_shelly_power_from_url(url: str):
    try:
...
def build_training_command(gpu_id: int, duration_s: int):
    return ["bash", "-lc", f"CUDA_VISIBLE_DEVICES={gpu_id} {GPU_BURN_PATH} {duration_s}"]
...
def should_defer(policy_name: str, job: Job, phase_elapsed_s: float, phase_seconds: int, telemetry: Dict[int, Dict]):
    ...
    now_phase_s = phase_elapsed_s
    latest_allowed = job.arrival_offset_s + job.defer_slack_s
    if now_phase_s >= latest_allowed:
        return False, ""

    if in_peak_window(now_phase_s, phase_seconds):
...
def run_phase(phase_name: str, policy_name: str, phase_seconds: int, jobs: List[Job], event_writer, ts_writer):
    ...
    wall_cost = 0.0
    peak_energy_shifted_kwh = 0.0
    baseline_peak_energy_would_have_run_ws = 0.0
        ...
        while time.time() < phase_end_abs or running_jobs or job_idx < len(jobs) or queue:
            ...
            for job in list(queue):
                ...
                if defer:
                    ...
                    if compute_power is not None and in_peak_window(phase_elapsed_s, phase_seconds) and job.job_type == "training":
                        baseline_peak_energy_would_have_run_ws += compute_power * TICK_SECONDS
                ...
                if policy_name != "baseline" and job.flexible and not in_peak_window(phase_elapsed_s, phase_seconds):
                    # rough shifted-energy estimate for flexible jobs launched outside mini-peak
                    if compute_power is not None:
                        shifted_kwh = (compute_power * min(job.requested_duration_s, TICK_SECONDS)) / 3_600_000.0
                        peak_energy_shifted_kwh += shifted_kwh
    ...
    return {
        ...
        "hot_samples_ge_70c": int(hot_samples),
        ...
        "peak_energy_shifted_kwh": round(peak_energy_shifted_kwh, 6),
    }
...
def main():
    ...
    # Use identical workload template in both phases
    random.seed(SEED)
    ...
    make_owner_plots(summary_df, pd.read_csv(TIME_SERIES_CSV), pd.read_csv(EVENTS_CSV))
...
def make_owner_plots(summary_df: pd.DataFrame, ts_df: pd.DataFrame, events_df: pd.DataFrame):
    ...
    # 2. Cost over time
    if {"wall_power_w", "energy_price_per_kwh", "phase", "timestamp"}.issubset(ts_df.columns):
        ...
        plt.title("Cumulative Energy Cost: With vs Without Scheduler")
        plt.xlabel("Time")
        plt.ylabel("Cost ($)")
        plt.legend()
    ...
    # 3. PUE comparison over time
    if {"timestamp", "phase", "pue_proxy"}.issubset(ts_df.columns):
        plt.figure(figsize=(12, 5))
        for phase in ["baseline", "scheduled"]:
            sub = ts_df[ts_df["phase"] == phase]
            plt.plot(sub["timestamp"], sub["pue_proxy"], label=phase)
        plt.title("PUE Proxy Over Time: With vs Without Scheduler")
        plt.xlabel("Time")
    ...
    # 4. Peak temperatures avoided
    temp_cols = ["gpu0_temp_c", "gpu1_temp_c", "gpu2_temp_c"]
    if set(temp_cols + ["phase"]).issubset(ts_df.columns):
        temp_long = []
        for gpu_col in temp_cols:
            temp_long.append(ts_df[["timestamp", "phase", gpu_col]].rename(columns={gpu_col: "temp_c"}))
        temp_long = pd.concat(temp_long, ignore_index=True)

        phase_peaks = temp_long.groupby("phase", as_index=False)["temp_c"].max()
        plt.figure(figsize=(8, 5))
        plt.bar(phase_peaks["phase"], phase_peaks["temp_c"])
        plt.title("Peak GPU Temperature Avoided")
        plt.xlabel("Condition")
        plt.ylabel("Peak Temperature (C)")
        save_plot("04_peak_temperature_avoided.png")

    # 5. Wall energy and compute energy
    energy_comp = summary_df[["phase", "total_wall_energy_kwh", "total_compute_energy_kwh"]].copy()
    plt.figure(figsize=(9, 5))
    x = range(len(energy_comp))
    width = 0.35
    plt.bar([i - width/2 for i in x], energy_comp["total_wall_energy_kwh"], width=width, label="Wall Energy")
    plt.bar([i + width/2 for i in x], energy_comp["total_compute_energy_kwh"], width=width, label="Compute Energy")
    plt.xticks(list(x), energy_comp["phase"])
    plt.title("Energy Consumption Comparison")
    plt.ylabel("kWh")
    plt.legend()
    save_plot("05_energy_comparison.png")

    # 6. Energy shifted out of peak
    shifted = summary_df[["phase", "peak_energy_shifted_kwh"]].copy()
    plt.figure(figsize=(8, 5))
    plt.bar(shifted["phase"], shifted["peak_energy_shifted_kwh"])
    plt.title("Energy Shifted Out of Mini-Peak Window")
    plt.ylabel("Shifted Energy (kWh)")
    save_plot("06_peak_energy_shifted.png")

    # 7. Thermal stress comparison
    thermal = summary_df[["phase", "hot_samples_ge_70c", "peak_temp_c"]].copy()
    plt.figure(figsize=(10, 5))
    x = range(len(thermal))
    width = 0.35
    plt.bar([i - width/2 for i in x], thermal["hot_samples_ge_70c"], width=width, label="Hot Samples >=70C")
    plt.bar([i + width/2 for i in x], thermal["peak_temp_c"], width=width, label="Peak Temp (C)")
    plt.xticks(list(x), thermal["phase"])
    plt.title("Thermal Stress Comparison")
    plt.legend()
    save_plot("07_thermal_stress_comparison.png")

    # 8. Scaled data center impact estimate
    ...
    # 9. Dispatch summary owners care about
    if {"phase", "event"}.issubset(events_df.columns):
