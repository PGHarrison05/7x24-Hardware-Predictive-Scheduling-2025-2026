...
TOTAL_TEST_MINUTES = 120          # total test time
BASELINE_FRACTION = 0.50          # first half baseline, second half thermal scheduler
TICK_SECONDS = 5
SEED = 42
...
RUN_NAME = f"thermal_stress_v2_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
...
def main():
    ...
    peak_reduction_pct = (100 * peak_reduction / baseline["peak_temp_c"]) if baseline["peak_temp_c"] else 0.0
    avg_reduction_pct = (100 * avg_reduction / baseline["avg_gpu_temp_c"]) if baseline["avg_gpu_temp_c"] else 0.0
    hot_reduction_pct = (100 * hot_reduction / baseline["hot_samples_ge_55c"]) if baseline["hot_samples_ge_55c"] else 0.0

    print(f"Peak temp (baseline):            {baseline['peak_temp_c']:.3f} C")
    print(f"Peak temp (thermal):             {thermal['peak_temp_c']:.3f} C")
    print(f"Peak temp reduction:             {peak_reduction:.3f} C")
    print(f"Peak temp reduction percent:     {peak_reduction_pct:.2f}%\n")

    print(f"Avg temp (baseline):             {baseline['avg_gpu_temp_c']:.3f} C")
    print(f"Avg temp (thermal):              {thermal['avg_gpu_temp_c']:.3f} C")
    print(f"Avg temp reduction:              {avg_reduction:.3f} C")
    print(f"Avg temp reduction percent:      {avg_reduction_pct:.2f}%\n")

    print(f"Hot samples >=55C (baseline):    {baseline['hot_samples_ge_55c']:.0f}")
    print(f"Hot samples >=55C (thermal):     {thermal['hot_samples_ge_55c']:.0f}")
    print(f"Hot samples avoided:             {hot_reduction:.0f}")
    print(f"Hot samples avoided percent:     {hot_reduction_pct:.2f}%\n")

    print(f"Thermal defers used:             {thermal['thermal_defers']:.0f}")
    print(f"Jobs completed baseline:         {baseline['jobs_completed']:.0f}")
    print(f"Jobs completed thermal:          {thermal['jobs_completed']:.0f}")
    print(f"Avg launch delay baseline:       {baseline['avg_delay_s']:.3f} s")
    print(f"Avg launch delay thermal:        {thermal['avg_delay_s']:.3f} s\n")
