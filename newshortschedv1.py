...
BASELINE_FRACTION = 0.50   # first half baseline, second half scheduled
...
# -----------------------------
# Launch throttling
# keeps scheduled jobs from all bunching at once
# -----------------------------
...
RUN_NAME = f"demo_compare_v2_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
...
def run_phase(phase_name: str, policy_name: str, phase_seconds: int, jobs: List[Job], event_writer, ts_writer):
    ...
    return {
        ...
        "hot_samples_ge_66c": int(hot_samples),
...
def make_owner_plots(summary_df: pd.DataFrame, ts_df: pd.DataFrame, events_df: pd.DataFrame):
    ...
    width = 0.35
    plt.bar([i - width/2 for i in x],
            [baseline["hot_samples_ge_66c"], scheduled["hot_samples_ge_66c"]],
            width=width, label="Hot Samples >=66C")
    plt.bar([i + width/2 for i in x],
...
def main():
    ...
    # identical workload pattern in both phases
    random.seed(SEED)
